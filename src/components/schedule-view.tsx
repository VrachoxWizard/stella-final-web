...
const fullDate = new Intl.DateTimeFormat("hr-HR", { weekday: "long", day: "numeric", month: "long", year: "numeric", timeZone: "Europe/Zagreb" });
const shortDate = new Intl.DateTimeFormat("hr-HR", { day: "numeric", month: "short", timeZone: "Europe/Zagreb" });
...
export function ScheduleView({ matches }: { matches: Match[] }) {
  ...
  const ageGroups = useMemo(() => [...new Set(matches.map((match) => match.ageGroup))].sort(), [matches]);
  ...
  return <>
    <div className="schedule-controls">
      <div className="filter-bar" aria-label="Filtriraj raspored po uzrastu">{[["svi", "Svi uzrasti"], ...ageGroups.map((year) => [year, year])].map(([value, label]) => <button key={value} type="button" aria-pressed={ageFilter === value} className={ageFilter === value ? "active" : ""} onClick={() => setAgeFilter(value)}>{label}</button>)}</div>
